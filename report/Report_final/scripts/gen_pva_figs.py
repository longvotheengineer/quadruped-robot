...
# Generate time vector for 3 cycles: [-1, 0] Stance, [0, 1] Swing, [1, 2] Stance
...
def plot_pva_extended(tau, s, v, a, title_prefix, color, filename, s_label, v_label, a_label):
    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(8, 10))

    # 1. Position Plot
    ax1.plot(tau, s, color=color, linewidth=2, label='Vị trí')
    ax1.set_ylabel('$s(\\tau)$')
    ax1.grid(True, linestyle=':', alpha=0.7)
    ax1.set_xlim(-0.5, 1.5)
    ax1.axvspan(-0.5, 0, color='gray', alpha=0.1)
    ax1.axvspan(1, 1.5, color='gray', alpha=0.1)
    ax1.text(-0.25, 0.5, 'Pha chống\n(Stance)', ha='center', va='center', alpha=0.5)
    ax1.text(1.25, 0.5, 'Pha chống\n(Stance)', ha='center', va='center', alpha=0.5)
    ax1.text(0.5, 0.5, 'Pha bay\n(Swing)', ha='center', va='center', alpha=0.5)

    # 2. Velocity Plot
    ax2.plot(tau, v, color=color, linewidth=2, label='Vận tốc')
    ...
    # 3. Acceleration Plot
    ax3.plot(tau, a, color=color, linewidth=2, label='Gia tốc')
...
plot_pva_extended(tau_full, s3, v3, a3, "Đa thức bậc 3", 'blue', 'img/fig_cubic_pva.pdf', s_label='Cubic $s$', v_label='Cubic $\\dot{s}$', a_label='Cubic $\\ddot{s}$')
plot_pva_extended(tau_full, s5, v5, a5, "Đa thức bậc 5", 'red', 'img/fig_quintic_pva.pdf', s_label='Quintic $s$', v_label='Quintic $\\dot{s}$', a_label='Quintic $\\ddot{s}$')
