...
Tổng khối lượng robot được ước tính xấp xỉ $M = 3.5\text{ kg}$.
...
\begin{equation}
    F_N = \frac{Mg}{2}
\end{equation}
Thay các giá trị từ Bảng \ref{tab:2} với $M = 3.5\,\text{kg}$, $L_3 = 0.125\,\text{m}$, $L_{DE} = 0.024\,\text{m}$, $L_{CB} = 0.025\,\text{m}$, hệ số tải động $\eta = 1.2$, góc nghiêng $\alpha = 45^\circ$, và $g = 9.81\,\text{m/s}^2$ vào công thức, ta có $F_N = 17.17\,\text{N}$ và:
\begin{equation}
    T_{\text{knee}} \approx 1.2 \times 17.17 \times 0.125 \times \sin(45^\circ) \times \frac{0.024}{0.025} \approx 1.748\,\text{N}\cdot\text{m} \approx 17.8\,\text{kg}\cdot\text{cm}
\end{equation}
...
Tương tự phân tích trước, tải trọng lớn nhất tác dụng lên một chân là $F_N = 17.17\,\text{N}$, hệ số tải động $\eta = 1.2$, dẫn đến lực tác dụng tổng cộng là $\eta F_N = 20.6\,\text{N}$. 
...
\begin{equation}
    T_{roll} = 20.6 \times \big(0.026 \cos(\theta_1) + 0.164 \sin(\theta_1)\big) \quad \text{(N}\cdot\text{m)}
\end{equation}

\begin{itemize}
    \item Khi $\theta_1 = 0^\circ$, mô-men tác dụng lên khớp vai chỉ xuất phát từ khoảng lệch ngàm cơ khí: $T_{roll} = 20.6 \times 0.026 \approx 0.54\,\text{N}\cdot\text{m} \approx 5.5\,\text{kg}\cdot\text{cm}$.
    \item Khi $\theta_1 = 15^\circ$, mô-men tăng lên: $T_{roll} \approx 1.39\,\text{N}\cdot\text{m} \approx 14.2\,\text{kg}\cdot\text{cm}$, phản ánh trạng thái robot điều chỉnh tư thế rẽ với góc dang trung bình.
    \item Khi $\theta_1 = 30^\circ$, mô-men đạt giá trị lớn nhất: $T_{roll} \approx 2.15\,\text{N}\cdot\text{m} \approx 21.9\,\text{kg}\cdot\text{cm}$, tương ứng với tình huống bất lợi khi chân dang rộng.
\end{itemize}

Giá trị mô-men cực đại vẫn nhỏ hơn định mức $30\,\text{kg}\cdot\text{cm}$ của servo ST3215, do đó khớp vai vẫn hoạt động trong vùng an toàn, không gây quá tải.
...
\begin{equation}
    T_{pitch} = 20.6 \times 0.164 \sin(\alpha) = 3.378 \sin(\alpha) \quad \text{(N}\cdot\text{m)}
\end{equation}
...
\begin{itemize}
    ...
    \item Khi chân bước với góc $\alpha = 15^\circ$, mô-men tại khớp hông đạt $T_{pitch} = 3.378 \sin(15^\circ) \approx 0.87\,\text{N}\cdot\text{m} \approx 8.9\,\text{kg}\cdot\text{cm}$. Đây là vùng tải nhẹ, thuận lợi cho động cơ.
    \item Khi $\alpha = 45^\circ$, mô-men tăng lên $T_{pitch} = 3.378 \sin(45^\circ) \approx 2.39\,\text{N}\cdot\text{m} \approx 24.4\,\text{kg}\cdot\text{cm}$, tương ứng với trạng thái vươn dài chân để leo dốc hoặc vượt chướng ngại vật.
\end{itemize}

Giá trị cực đại này vẫn nhỏ hơn đáng kể so với giới hạn mô-men của servo ST3215 ($30\,\text{kg}\cdot\text{cm}$), xác nhận rằng việc lựa chọn động cơ cho khớp hông là hoàn toàn phù hợp và đảm bảo độ tin cậy trong vận hành.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{hardware/power_supply_12v.png} 
    \caption{Bộ nguồn xung tổ ong 12~V 40~A 480~W}
    \label{fig35}
\end{figure}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textit{Khớp vai và khớp hông (Hip Roll, Hip Pitch):} Gánh chịu phần lớn trọng lượng của thân robot và tạo ra lực đẩy chính, yêu cầu mô-men xoắn lớn (theo tính toán lý thuyết lên tới 21.9 -- 24.4 kg.cm). Do đó, phiên bản tiêu chuẩn \textbf{ST3215 (30 kg.cm)} được sử dụng để tạo ra hệ số an toàn, giúp động cơ hoạt động bền bỉ, không sinh nhiệt quá mức khi robot đứng khuỵu hoặc mang thêm tải trọng.
        \item \textit{Khớp gối (Knee):} Yêu cầu về lực tại khớp gối thấp hơn đáng kể (khoảng 17.8 kg.cm) nhờ cơ cấu tay đòn và thanh liên kết. Tuy nhiên, khớp gối lại đòi hỏi tốc độ gập/duỗi cực kỳ nhanh để chân có thể nhấc lên và vung về phía trước (Swing phase) trong thời gian rất ngắn. Vì vậy, phiên bản tốc độ cao \textbf{ST3215-HS (20 kg.cm)} được lựa chọn. Dù mô-men xoắn giảm đi một phần nhưng vẫn vượt qua ngưỡng an toàn, đổi lại tốc độ phản hồi đạt $0.094\,\text{s}/60^{\circ}$ (nhanh gấp đôi bản thường), giúp robot di chuyển linh hoạt và lấy lại thăng bằng chớp nhoáng khi gặp nhiễu.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item \textbf{Chức năng:} Truyền tải toàn bộ dòng điện từ nguồn xung 12~V - 40~A đến các khối chia nguồn trung tâm.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{control_sys/communication_protocols.png} 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{control_sys/i2c_data_frame.png} 
    \caption{Giao thức truyền thông I2C}
    \label{fig40}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{control_sys/wired_uart.png} 
