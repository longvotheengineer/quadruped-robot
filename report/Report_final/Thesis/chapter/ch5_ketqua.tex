...
\begin{figure}[H]
    ...
    \label{fig4}
\end{figure}
Hình \ref{fig4} trình bày ba góc khớp đầu ra của thuật toán IK ($\theta_1$, $\theta_2$, $\theta_3$) theo đơn vị độ cho cả bốn chân trong một chu kỳ dáng đi đầy đủ gồm $T_{\text{total}} = 340$ waypoints ($T_{\text{swing}} = 30$, $T_{\text{stance}} = 310$).
...
\begin{figure}[H]
    ...
    \label{fig5}
\end{figure}
Hình \ref{fig5} trình bày kết quả ước lượng mô-men xoắn cho chân Left-Front (chân tiêu biểu). Mô hình sử dụng luật điều khiển PD kết hợp bù trọng lực (gravity compensation):
\begin{equation}
...
\begin{figure}[H]
    ...
    \label{fig6}
\end{figure}
Hình \ref{fig6} hiển thị giản đồ thời gian (timing diagram) của dáng đi Trot cho bốn chân, trong đó thanh màu xanh biểu thị pha swing và vùng trắng biểu thị pha stance.
