...
\textbf{Hiệu chỉnh bộ điều khiển trục Roll:}
...
\textbf{Hiệu chỉnh bộ điều khiển trục Pitch:}
...
\begin{itemize}
    \item \textbf{Độ vọt lố (Overshoot):} Cả hai trục Roll và Pitch đều có độ vọt lố rất nhỏ. Khi bù nhiễu, góc vượt qua mốc $0$, đạt giá trị âm lớn nhất khoảng $-0{,}005$ rad. Mức vọt lố này nằm trong giới hạn vô cùng an toàn, đảm bảo robot không bị mất trọng tâm dẫn đến lật ngã.
    \item \textbf{Thời gian quá độ (Settling Time):} Hệ thống mất khoảng $7-9$ s (từ $t = 3$ s đến $t = 12$ s) để dập tắt hoàn toàn các dao động và dao động tắt dần quanh giá trị cân bằng. Thời gian phản hồi này cho thấy hệ thống có khả năng đáp ứng khá nhanh trước các thay đổi bất ngờ.
    \item \textbf{Sai số xác lập (Steady-state Error):} Từ sau mốc $t = 12$ s, hệ thống hoàn toàn duy trì được trạng thái thăng bằng với sai số tĩnh xấp xỉ bằng $0$. Điều này minh chứng thành phần khâu tích phân (I) trong bộ điều khiển PID đã hoạt động hiệu quả để triệt tiêu các sai lệch dư.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Độ vọt lố (Overshoot):} Khác với PID hoàn chỉnh, trường hợp bù trực tiếp hoàn toàn \textbf{không có độ vọt lố} qua mốc cân bằng $0$. Góc nghiêng đo được giảm dần nhưng chưa bao giờ đạt tới hay vượt qua mốc $0$ rad. 
    ...
    \item \textbf{Sai số xác lập (Steady-state Error):} Đây là hạn chế nghiêm trọng nhất của phương pháp bù trực tiếp vòng kín. Sau thời gian quá độ, góc nghiêng xác lập ở mức khá cao, dao động quanh $0{,}07$ rad (đối với Roll) và $0{,}05$ rad (đối với Pitch). Hệ thống vòng kín tỷ lệ 1:1 không thể bù trừ triệt để lượng nhiễu đầu vào. Để triệt tiêu hoàn toàn sai số này và đưa hệ thống về điểm làm việc chuẩn, bắt buộc phải tích hợp khâu tích phân (I) nhằm duy trì tín hiệu bù kể cả khi góc nghiêng tiến về $0$.
\end{itemize}
...
\begin{itemize}
    \item Trước thời điểm $t = 3$ s, cả 4 chân đều có chung một biên dạng ổn định: pha chống chân nằm ở mức âm cơ sở, và pha vung chân tạo thành các đỉnh nhọn nhấc lên đều đặn.
    \item Từ thời điểm $t = 3$ s, khi có nhiễu nghiêng xuất hiện, đồ thị có sự phân hóa rõ rệt. Đặc biệt, có đường cơ sở nhảy vọt và các đỉnh vung chân lại hướng xuống mạnh mẽ.
    \item \textbf{Nguyên lý động học (Kinematic Coupling):} Hiện tượng trên xảy ra do sự cộng hưởng của nhiễu Roll và Pitch. Khi robot bị nghiêng mạnh, có điểm chân bị ép sát xuống mặt đất nhất. Để bù trừ, PID ra lệnh thu chân lại cực hạn. Trong tư thế gập sâu này, kết hợp với việc hệ trục tọa độ thân robot đã bị nghiêng lớn, động tác vung chân về phía trước bị "khớp nối động học" và chuyển hóa thành chuyển động duỗi dài ra theo trục Z của thân robot để có thể chạm tới điểm đặt chân mong muốn. Điều này minh họa rõ rệt giới hạn không gian làm việc (workspace boundary) và phản ứng động học phức tạp của robot bốn chân dưới tác động của sai lệch tư thế lớn.
\end{itemize}
...
\begin{itemize}
    \item \textbf{\texttt{roll\_meas} / \texttt{pitch\_meas}:} Tín hiệu góc đo được bao gồm các dao động tuần hoàn với biên độ lớn do sự dịch chuyển trọng tâm luân phiên giữa các chân khi bước đi. 
    ...
    \item \textbf{\texttt{after Avg}:} Bộ lọc trung bình (Avg) đóng vai trò then chốt như một bộ lọc thông thấp (Low-pass Filter). Tại $t = 3$ s, khi nhiễu bước tác động vào hệ thống, bộ lọc Avg đã triệt tiêu phần lớn dao động của dáng đi và trích xuất thành công độ trôi dạt tư thế (baseline drift). 
    \item \textbf{\texttt{after Deadzone}:} Tín hiệu qua khâu vùng chết (Deadzone) để cắt xén phần vi sai dư thừa, minh chứng rằng bộ điều khiển đã trích xuất được mức sai số tư thế hữu ích và chính xác để đưa vào khâu tính toán PID, giúp loại bỏ hoàn toàn hiện tượng phản ứng quá mức với các dao động tự nhiên.
\end{itemize}
