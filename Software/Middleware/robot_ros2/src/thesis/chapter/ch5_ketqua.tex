...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.19\textwidth}
        \centering
        \includegraphics[width=\textwidth]{real_robot/frame_001.jpg}
        \caption{0.0s}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.19\textwidth}
        \centering
        \includegraphics[width=\textwidth]{real_robot/frame_002.jpg}
        \caption{1.3s}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.19\textwidth}
        \centering
        \includegraphics[width=\textwidth]{real_robot/frame_003.jpg}
        \caption{2.7s}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.19\textwidth}
        \centering
        \includegraphics[width=\textwidth]{real_robot/frame_004.jpg}
        \caption{4.0s}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.19\textwidth}
        \centering
        \includegraphics[width=\textwidth]{real_robot/frame_005.jpg}
        \caption{5.3s}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{0.19\textwidth}
        \centering
        \includegraphics[width=\textwidth]{real_robot/frame_006.jpg}
        \caption{6.7s}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.19\textwidth}
        \centering
        \includegraphics[width=\textwidth]{real_robot/frame_007.jpg}
        \caption{8.0s}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.19\textwidth}
        \centering
        \includegraphics[width=\textwidth]{real_robot/frame_008.jpg}
        \caption{9.3s}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.19\textwidth}
        \centering
        \includegraphics[width=\textwidth]{real_robot/frame_009.jpg}
        \caption{10.7s}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.19\textwidth}
        \centering
        \includegraphics[width=\textwidth]{real_robot/frame_010.jpg}
        \caption{12.0s}
    \end{subfigure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{gait_planner/foot_trajectory_2d.png}
    \caption{Quỹ đạo bàn chân trong không gian 2D}
    \label{fig1}
\end{figure}
Hình \ref{fig1} trình bày quỹ đạo bàn chân trong mặt phẳng XZ (mặt phẳng sagittal), so sánh hai phương pháp lập kế hoạch: sóng sin (sine-wave) và đa thức bậc 5 (quintic polynomial).

Quỹ đạo bàn chân có dạng hình chữ D, bao gồm hai pha: pha vung chân (swing) trong đó bàn chân di chuyển theo đường cong từ điểm A đến điểm D với độ nâng $h = 80$ mm so với mặt đất, và pha chống chân (stance) trong đó bàn chân trượt ngược trên mặt đất tạo lực đẩy cho robot.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{gait_planner/foot_trajectory_3d.png}
    \caption{Quỹ đạo bàn chân trong không gian 3D}
    \label{fig2}
\end{figure}
Hình \ref{fig2} hiển thị quỹ đạo 3D của cả bốn chân trong hệ tọa độ thân robot (body frame), với kích thước thân $L \times W = 209 \times 191$ mm. Bốn quỹ đạo hình chữ D được phân bố tại bốn góc của khung thân, mỗi chân có offset pha tương ứng với dáng đi Trot. Dáng đi này có đặc điểm hai chân chéo nhau cùng thực hiện pha vung chân (LF+RB và LB+RF luân phiên), đảm bảo tại mọi thời điểm luôn có ít nhất hai chân chạm đất.

\subsection{Biên dạng vận tốc và gia tốc}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{gait_planner/velocity_acceleration.png}
    \caption{Biên dạng vận tốc và gia tốc}
    \label{fig3}
\end{figure}
Hình \ref{fig3} so sánh biên dạng vận tốc và gia tốc của bàn chân theo hai trục X và Z giữa hai phương pháp, gồm bốn subplot:

Subplot (a) và (b) thể hiện vận tốc theo trục X ($dx/dt$) và trục Z ($dz/dt$). Kết quả cho thấy phương pháp đa thức bậc 5 có vận tốc bằng 0 tại cả hai đầu pha vung chân, trong khi phương pháp sóng sin có bước nhảy vận tốc tại ranh giới chuyển pha vung - chống chân.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{gait_planner/fifth_degree_profiling.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{gait_planner/joint_angles.png}
    \caption{Biến thiên góc khớp}
    \label{fig4}
\end{figure}
Hình \ref{fig4} trình bày ba góc khớp đầu ra của thuật toán IK ($\theta_1$, $\theta_2$, $\theta_3$) theo đơn vị độ cho cả bốn chân trong một chu kỳ dáng đi đầy đủ gồm $T_{\text{total}} = 245$ waypoints ($T_{\text{swing}} = 45$, $T_{\text{stance}} = 200$).
...
Góc $\theta_3$ (khớp tibia/gối) thay đổi rõ rệt trong pha vung chân khi bàn chân nâng lên để tránh va chạm mặt đất, với biên độ thay đổi khoảng $50^\circ$ so với vị trí chống chân. Hiệu ứng offset pha của dáng đi Trot được quan sát rõ: cặp LF+RB có pha vung chân tại waypoint 0–45, cặp LB+RF có pha vung chân tại waypoint 122–167 (lệch 50\% chu kỳ).

\subsection{Ước lượng mô-men xoắn bộ điều khiển PD}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{control_sys/pd_torque_control.png}
    \caption{Mô-men xoắn bộ điều khiển PD}
    \label{fig5}
\end{figure}
Hình \ref{fig5} trình bày kết quả ước lượng mô-men xoắn cho chân Left-Front (chân tiêu biểu). Mô hình sử dụng luật điều khiển PD:
\begin{equation}
    \tau_{\text{total}} = K_p \cdot (\theta_{\text{target}} - \theta_{\text{actual}}) - K_d \cdot \omega_{\text{actual}}
\end{equation}
Vị trí khớp thực tế ($\theta_{\text{actual}}$) được mô hình hóa như phiên bản lọc thông thấp (low-pass filter) của vị trí mong muốn, mô phỏng độ trễ bám theo (tracking delay) đặc trưng của servo.
...
\begin{itemize}
    ...
    \item \textbf{Khớp 2 (Femur):} Mô-men lớn nhất đạt khoảng $2,8$ Nm tại thời điểm bắt đầu pha vung chân, khi chân phải tăng tốc nhanh để nâng lên. Giá trị này nằm trong giới hạn effort của URDF ($\pm 2,94$ Nm), xác nhận servo đủ công suất.
    \item \textbf{Khớp 3 (Tibia):} Mô-men dao động trong khoảng $0,5\text{--}0,7$ Nm, nằm an toàn trong giới hạn ($\pm 1,96$ Nm).
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{gait_planner/gait_timing.png}
    \caption{Gait timing}
    \label{fig6}
\end{figure}
Hình \ref{fig6} hiển thị giản đồ thời gian (timing diagram) của dáng đi Trot cho bốn chân, trong đó thanh màu xanh biểu thị pha vung chân và vùng trắng biểu thị pha chống chân.

Dáng đi Trot có hệ số chu kỳ nhiệm vụ $\beta = T_{\text{stance}} / T_{\text{total}} = 200/245 \approx 81{,}6\%$. Điều này có nghĩa mỗi chân ở trạng thái chống chân (stance) khoảng 81,6\% thời gian và ở trạng thái vung chân (swing) 18,4\% thời gian. Giá trị $\beta > 0{,}5$ đảm bảo tại mọi thời điểm luôn có ít nhất hai chân chạm đất. Hai cặp chân chéo nhau (LF+RB và LB+RF) luân phiên thực hiện pha vung chân với độ lệch pha 50\% chu kỳ.

\subsection{Đa giác đỡ và ổn định tĩnh}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{kinematics/support_polygon.png}
    \caption{Đa giác đỡ và ổn định tĩnh}
    \label{fig7}
\end{figure}
Hình \ref{fig7} phân tích ổn định tĩnh bằng phương pháp đa giác đỡ (Support Polygon) tại hai thời điểm đặc trưng trong chu kỳ dáng đi.

Tại thời điểm $t = 0$ (pha vung chân của cặp LF+RB), chỉ có hai chân LB và RF chạm đất. Đa giác đỡ suy biến thành đoạn thẳng nối hai bàn chân. Hình chiếu trọng tâm (Center of Mass — CoM) của robot xuống mặt phẳng đỡ nằm gần đường thẳng này, cho thấy ổn định ở mức biên (marginal stability).

Tại thời điểm $t = T/4$ (pha chống chân đầy đủ), cả bốn chân đều chạm đất. Đa giác đỡ là hình chữ nhật bao quanh robot với diện tích lớn. Hình chiếu CoM nằm hoàn toàn bên trong đa giác đỡ, cho thấy ổn định tĩnh cao.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{kinematics/workspace.png}
    \caption{Không gian làm việc}
    \label{fig8}
\end{figure}
Hình \ref{fig8} trình bày không gian làm việc (workspace) của một chân robot 3-DOF, tính bằng phương pháp quét động học thuận (Forward Kinematics sweep).
...
Kết quả cho thấy workspace có dạng \textbf{vòng cung} (annular sector), đặc trưng cho cơ cấu serial 3-DOF. Vùng đạt được rộng nhất trong mặt phẳng X-Z (sagittal), trong khi chiều ngang theo phương Y bị giới hạn bởi biên độ khớp hông (hip - $\theta_1$). Tầm với tối đa của chân là $l_2 + l_3 = 106 + 125 = 231$ mm, tương ứng với trường hợp xương đùi và xương ống chân duỗi thẳng hoàn toàn.
...
Thứ hai, bộ điều khiển PD với các tham số $K_p = 13{,}0$ Nm/rad và $K_d = 0{,}3$ Nm$\cdot$s/rad tạo ra mô-men xoắn nằm trong giới hạn cho phép của servo tại cả ba khớp. Mô-men lớn nhất xuất hiện tại khớp femur ($\sim 2{,}8$ Nm) vẫn thấp hơn giới hạn URDF ($2{,}94$ Nm), xác nhận bộ truyền động đủ công suất cho dáng đi Trot.

Thứ ba, dáng đi Trot với duty cycle $\beta \approx 81{,}6\%$ duy trì ổn định tĩnh trong suốt chu kỳ. Phân tích đa giác đỡ cho thấy biên ổn định nhỏ nhất xảy ra tại thời điểm chuyển pha khi chỉ có hai chân chạm đất, nhưng vẫn đảm bảo hình chiếu trọng tâm nằm trong vùng đỡ.

Thứ tư, không gian làm việc dạng vòng cung với tầm với tối đa 231 mm đủ lớn để thực hiện các bước đi trong phạm vi thiết kế. Kiến trúc phần mềm phân tầng (trajectory planning --- low-level control --- plant) cho phép mô phỏng trước khi triển khai trên phần cứng thực với thay đổi tối thiểu.
...
Hình~\ref{results/offline_traj_pos} trình bày quỹ đạo Cartesian $(x, y, z)$ của chân trước bên trái (Left-Front) trong một chu kỳ dáng đi đầy đủ gồm pha vung chân ($T_{swing} = 45$ frame) và pha chống chân ($T_{stance} = 200$ frame). Quỹ đạo trục $x$ thể hiện rõ chuyển động tiến/lùi với biên độ bước $L_{stride} = 10$ mm, trục $y$ duy trì ổn định, và trục $z$ cho thấy biên dạng nâng chân dạng parabol với độ cao $h_{lift} = 80$ mm.
...
Hình~\ref{results/offline_traj_vel_acc} trình bày biên dạng vận tốc và gia tốc tương ứng, xác nhận điều kiện biên $\dot{s}(0) = \dot{s}(1) = 0$ và $\ddot{s}(0) = \ddot{s}(1) = 0$ của hàm nội suy quintic.
...
Hình~\ref{results/offline_traj_angle} cho thấy các góc khớp $(\theta_1, \theta_2, \theta_3)$ đã giải qua IK, với pha vung chân và pha chống chân được tô màu phân biệt. Các chuyển tiếp góc khớp mượt mà, không có bước nhảy đột ngột, xác nhận tính đúng đắn của thuật toán IK.
...
Hình~\ref{fig_gait_3d_real} trình bày quỹ đạo 3D của chân trước bên trái trên hệ thống mô phỏng, cho thấy rõ hình dạng chữ D đặc trưng với cung vung chân phía trên và đường chống chân phẳng phía dưới.
...
Hình~\ref{fig_pid_home} trình bày khả năng duy trì thăng bằng (đáp ứng sai lệch pitch và roll) của bộ điều khiển PID trong chế độ HOME khi chịu nhiễu liên tục. Để đánh giá hệ thống, mặt phẳng đỡ dưới chân robot được cấu hình dao động lượn sóng liên tục (Sinusoidal Platform Sway) với biên độ lớn (khoảng $0{,}17$ rad $\approx 10^\circ$). Dao động này sinh ra mô-men xoắn nhiễu ($\tau_{\text{disturbance}}$) liên tục truyền ngược từ chân lên thân robot. Đồ thị cho thấy rõ: mặc dù nhiễu mặt đất (Roll/Pitch Disturbance) lắc lư rất mạnh theo hình sin, bộ điều khiển PID ở hệ thống vòng kín đã liên tục xử lý và xuất lệnh bù, giúp kìm hãm sai lệch định hướng của thân robot (Roll/Pitch Error) ở mức rất nhỏ (dao động dưới $\pm 0{,}05$ rad). Đặc biệt, các phân đoạn đồ thị Error bị "cắt phẳng" ở lân cận 0 xác nhận cơ chế deadband (vùng chết $\pm 0{,}02$ rad) hoạt động cực kỳ hiệu quả, giúp robot cân bằng tĩnh vững chắc mà không bị dao động chattering (lắc lắt nhắt) quanh điểm đặt. Thêm vào đó, cơ chế chống bão hòa khâu tích phân (Anti-windup) đảm bảo rằng lực kéo bù không bị cộng dồn vô hạn khi servo chạm ngưỡng giới hạn, giúp hệ thống triệt tiêu hoàn toàn hiện tượng văng lố (overshoot) khi mặt phẳng đổi chiều dao động.
...
Hình \ref{fig:home_pid_roll_meas} và Hình \ref{fig:home_pid_pitch_meas} minh họa quá trình xử lý tín hiệu đo lường và giới hạn đầu ra. Tín hiệu đo lường ban đầu từ IMU có chứa nhiễu tần số cao, được làm mượt qua bộ lọc trung bình trượt hàm mũ (EMA - Giai đoạn 1). Tiếp đó, vùng chết (deadband - Giai đoạn 2) $\pm 0{,}02$ rad loại bỏ các dao động vi mô không cần thiết. Tại đầu ra, tín hiệu bù được bão hòa (Giai đoạn 4) ở mức $\pm 1{,}5$ rad và làm mượt bằng bộ lọc thông thấp (LPF - Giai đoạn 5), giúp lệnh điều khiển gửi xuống các khớp không bị giật cục.
...
Trong chế độ di chuyển (GAIT), bộ điều khiển phải đối mặt với các nhiễu loạn có tính chu kỳ sinh ra từ bước chân chạm đất. Chuỗi xử lý được bổ sung các cơ chế thích nghi đặc thù để duy trì ổn định.

Hình \ref{fig:trot_pid_meas} minh họa cơ chế đường cơ sở trung bình trượt (Moving Average Baseline). Thay vì phản ứng với mọi dao động tần số cao do va chạm bước chân, hệ thống tính toán giá trị trung bình trượt của góc nghiêng để tạo ra một đường tham chiếu làm mượt. Điều này giúp bộ điều khiển bỏ qua các rung lắc tự nhiên của bản thân dáng đi và chỉ tập trung hiệu chỉnh khi toàn bộ thân robot thực sự bị lệch khỏi mặt phẳng ngang.
...
Sự phân rã các thành phần PID trong Hình \ref{fig:trot_pid_pid} cho thấy tác dụng rõ rệt của cơ chế reset tích phân tại giao điểm không (Zero Crossing Integral Reset). Mỗi khi sai lệch đổi dấu (ví dụ thân robot lắc lư qua lại trục 0), khâu I lập tức bị xóa về 0, giúp ngăn chặn hoàn toàn hiện tượng quá điều chỉnh dao động (overshoot) thường thấy ở các bộ điều khiển PID truyền thống. Thành phần P cũng thể hiện tính thích nghi thông qua việc tự động điều chỉnh hệ số khuếch đại khi biên độ sai lệch tăng cao.
...
Hình \ref{fig:trot_pid_sat_lpf} minh họa khâu bão hòa và lọc đầu ra. So với chế độ HOME, tín hiệu bù trong chế độ GAIT bị bão hòa ở mức hẹp hơn ($u_{max} = 0{,}30$ rad) và bộ lọc thông thấp (LPF) hoạt động với cường độ lớn hơn rất nhiều ($\alpha = 0{,}97$). Thiết kế này đảm bảo các lệnh bù tư thế không được sinh ra quá gắt, ngăn chặn việc làm biến dạng quỹ đạo sải bước (stride trajectory) của các chân đang ở pha chống.
