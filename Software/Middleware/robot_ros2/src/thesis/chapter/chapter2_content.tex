...
Xét mô hình đơn giản hóa của robot bốn chân, trong đó toàn bộ khối lượng robot được tập trung tại trọng tâm CoM có tọa độ $(x, y, z)$ ở độ cao $z_0$ so với mặt đất. Điểm ZMP $(p_{x_k}, p_{y_k})$ nằm trên mặt phẳng đất, là điểm mà tại đó tổng mô-men của tất cả các lực tác dụng (bao gồm trọng lực và lực quán tính) bằng không.
...
trong đó $K_P$, $K_I$, và $K_D$ lần lượt là hệ số khuếch đại tỷ lệ, hệ số tích phân và hệ số vi phân. Mỗi thành phần đóng một vai trò riêng biệt trong việc định hình đáp ứng của hệ thống:

\begin{itemize}
    \item \textbf{Thành phần tỷ lệ (Proportional - P):} Tạo ra tín hiệu hiệu chỉnh tỷ lệ thuận với sai lệch hiện tại. Hệ số $K_P$ càng lớn, hệ thống phản ứng càng nhanh nhưng đồng thời có xu hướng dao động và độ vọt lố (overshoot) cao. Thành phần P đơn thuần không thể triệt tiêu hoàn toàn sai lệch ở trạng thái xác lập.

    \item \textbf{Thành phần tích phân (Integral - I):} Tích lũy sai lệch theo thời gian để triệt tiêu hoàn toàn sai số ở trạng thái xác lập (steady-state error). Tuy nhiên, khi sai lệch tồn tại trong thời gian dài do hệ thống bị bão hòa cơ cấu chấp hành, giá trị tích phân tích lũy quá lớn sẽ gây ra hiện tượng bão hòa tích phân (\textit{integral windup}), khiến hệ thống bị vọt lố nghiêm trọng và phục hồi chậm.

    \item \textbf{Thành phần vi phân (Derivative - D):} Dự đoán xu hướng thay đổi của sai lệch dựa trên đạo hàm. Thành phần này hoạt động như một ``phanh'' hãm, giúp giảm vọt lố và cải thiện thời gian xác lập. Nhược điểm chính của khâu D là rất nhạy cảm với nhiễu tần số cao trong tín hiệu đo đạc từ cảm biến.
\end{itemize}
...
\textbf{Chống bão hòa tích phân (Anti-windup).}
Khi đầu ra điều khiển bị bão hòa (vượt quá giới hạn vật lý của cơ cấu chấp hành), thành phần tích phân vẫn tiếp tục tích lũy sai số, gây ra hiện tượng windup. Để giải quyết vấn đề này, hai phương pháp phổ biến được sử dụng:
\begin{itemize}
    \item \textit{Clamping:} Khống chế trực tiếp giá trị tích phân không cho vượt qua một ngưỡng cố định, tức là $\mathcal{I}_{k} = \max(-\mathcal{I}_{max}, \min(\mathcal{I}_{max}, \mathcal{I}_{k}))$.
    \item \textit{Back-calculation:} Trừ ngược phần vượt giới hạn vào bộ tích phân khi đầu ra bão hòa: $\mathcal{I}_{k} \leftarrow \mathcal{I}_{k} - K_{bc}(u_{raw} - u_{sat})$, trong đó $K_{bc}$ là hệ số tính toán ngược.
\end{itemize}

\textbf{Vùng chết (Deadband/Deadzone).}
Trong thực tế, cảm biến luôn tồn tại nhiễu nhỏ ngay cả khi hệ thống đã ở vị trí cân bằng. Vùng chết triệt tiêu các sai lệch không đáng kể này để tránh động cơ phải hoạt động liên tục (chatter) gây mài mòn cơ khí:
\begin{equation}
    e_{dz} = \begin{cases}
        e - d & \text{nếu } e > d \\
        0 & \text{nếu } |e| \leq d \\
...
Để ngăn bộ tích phân giữ lại các sai số cũ quá lâu (làm hệ thống chậm chạp khi thay đổi điểm đặt), hệ số rò rỉ $\lambda \in (0, 1)$ được áp dụng vào khâu tích phân:
\begin{equation}
    \mathcal{I}_k = \lambda \cdot \mathcal{I}_{k-1} + e_k \cdot \Delta t, \quad \text{với } \lambda \approx 0{,}999
\end{equation}
Kỹ thuật này rất hữu ích cho các hệ thống có đặc tính thường xuyên thay đổi theo thời gian, giúp khâu tích phân luôn ``làm mới'' và phản ứng chính xác hơn với trạng thái hiện tại.
