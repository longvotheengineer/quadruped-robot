...
Khi thiết kế phương trình động học nghịch cho robot bốn chân, một điểm đặc biệt cần lưu ý là các chân trái và chân phải được lắp ráp đối xứng nhau như hình ảnh qua gương. Do tính chất này, ta không thể áp dụng chung một công thức toán học y hệt nhau cho cả 4 chân. Cụ thể, có ba yếu tố vật lý tạo ra sự khác biệt:

\begin{itemize}
    \item \textbf{Vị trí gắn chân trên thân:} Mỗi chân được gắn ở một góc khác nhau. Khoảng cách từ tâm robot đến các gốc chân có sự thay đổi về dấu, tương ứng với một nửa chiều dài ($\pm L/2$) và một nửa chiều rộng ($\pm W/2$).
    \item \textbf{Khớp hông (Coxa):} Do lắp đặt đối xứng, với cùng một góc quay, chân trái và chân phải sẽ vung về hai hướng ngược nhau. Để công thức toán học tính đúng hướng, tham số chiều dài khâu hông $L_1$ sẽ mang dấu âm ($-L_1$) đối với các chân bên trái và mang dấu dương ($+L_1$) đối với các chân bên phải.
    \item \textbf{Cấu hình gập gối (Tibia):} Để duy trì hình dáng gập gối đồng nhất cho cả hai bên (để gối không bị bẻ ngược), toán hạng $\sin\theta_3$ bắt buộc phải nhận hai dấu trái ngược nhau giữa chân trái ($-$) và chân phải ($+$).
\end{itemize}
...
\textbf{Quá trình giải hệ phương trình:} Sau khi biến đổi tọa độ, bài toán IK được giải thống nhất cho cả bốn chân thông qua bốn bước sau. Các hệ số dấu tham chiếu Bảng~\ref{tab:ik_signs}.
...
Hệ thống hoạt động dựa trên cơ chế phản hồi trạng thái động học như sau:
\begin{itemize}
    \item \textbf{Khâu đo lường và tạo sai số:} Cảm biến IMU (BNO055) liên tục đo lường góc nghiêng thực tế của thân robot (True Roll/Pitch). Các giá trị này được so sánh với góc tham chiếu lý thuyết (thường đặt $\phi_{ref}=0, \theta_{ref}=0$) thông qua các bộ cộng để tạo ra tín hiệu sai số $e_\phi$ và $e_\theta$.
    \item \textbf{Bộ điều khiển PID:} Sai số được đưa vào bộ điều khiển PID (tích hợp khâu chống vọt lố Anti-windup và lọc thông thấp LPF). Đầu ra của khối này là các tín hiệu bù cân bằng (Compensation Signals).
    \item \textbf{Chuyển mạch chế độ (Mode Switch):} Tùy thuộc vào trạng thái hiện tại của robot, hệ thống sẽ rẽ nhánh luồng tín hiệu bù:
    \begin{itemize}
        \item \textit{Chế độ di chuyển (GAIT Mode):} Tín hiệu bù được gửi tới khối Quy hoạch quỹ đạo và Động học nghịch (Trajectory Planning \& Inverse Kinematics) để tính toán bù trừ trực tiếp vào tọa độ không gian làm việc của bàn chân.
        \item \textit{Chế độ đứng yên (HOME Mode):} Tín hiệu bù được gửi tới khối Bù góc khớp trực tiếp (Direct Joint Angle Compensator) để tinh chỉnh không gian khớp.
    \end{itemize}
    \item \textbf{Thực thi:} Các góc khớp mục tiêu ($\theta_i$) được truyền xuống cơ cấu chấp hành (Bus Servo ST3215) để tác động lực lên đối tượng vật lý (Quadruped Robot Plant). Từ đó tạo ra các chuyển động triệt tiêu nhiễu loạn và giúp thân robot lấy lại tư thế ngang bằng.
\end{itemize}
...
Đặc điểm nổi bật của kiến trúc này là khả năng phân chia luồng xử lý độc lập cho trạng thái di chuyển (Gait Mode) và trạng thái đứng yên (Home Mode), kết hợp với cơ chế phản hồi thăng bằng linh hoạt:
\begin{itemize}
    \item \textbf{Bộ quy hoạch chuyển động (Locomotion Planner):} Là module chịu trách nhiệm tạo hình quỹ đạo chân khi robot di chuyển. Module này bao gồm Bộ lập lịch pha (Gait Phase Scheduler) để đồng bộ thời gian các chân, kết hợp với Bộ tạo quỹ đạo (Trajectory Generator) để sinh ra tọa độ bước chân dạng đa thức Quintic và D-shape. 
    \item \textbf{Khối bù tư thế không gian làm việc (Gait Attitude Compensator):} Nằm trong cụm Planner, khối này nhận tọa độ nguyên bản và cộng bù trực tiếp với tín hiệu chỉnh sửa $\Delta p$ từ bộ thăng bằng, sinh ra tọa độ chân hiệu chỉnh $p_{adj}$ trước khi đưa vào khâu giải động học nghịch (IK).
    \item \textbf{Bộ điều khiển cân bằng (Balance Controller):} Hoạt động độc lập, sử dụng dữ liệu góc Roll/Pitch từ IMU làm phản hồi cho bộ PID để liên tục tính toán lượng bù chống lại ngoại lực gây lật thân.
\end{itemize}

Sự linh hoạt của hệ thống nằm ở công tắc chuyển mạch điều chỉnh cân bằng (\textbf{Balance Output Switch}) và bộ chọn góc khớp cuối cùng (\textbf{Mode-based Final Angle Selector}). Tùy thuộc vào chế độ hoạt động hiện tại, tín hiệu bù cân bằng sẽ được định tuyến theo hai phương thức hoàn toàn khác biệt:

\begin{itemize}
    \item \textbf{Chế độ tĩnh (HOME) -- Bù trong không gian khớp (Joint Space):} 
    Khi robot đứng im, quỹ đạo làm việc bị khóa. Tín hiệu bù từ PID được chuyển mạch thành độ dịch góc khớp ($\Delta\theta_3$). Độ dịch này được bộ Final Angle Selector cộng trực tiếp vào góc khớp gối danh định $\theta_{k\_home}$ để gửi ngay xuống phần cứng (UART Serial Driver). Việc bỏ qua hoàn toàn cụm tính toán IK giúp phản ứng cân bằng đạt độ trễ tối thiểu, khử nhiễu ngoại lực tức thời.
...
Nhằm đáp ứng yêu cầu an toàn tuyệt đối cho phần cứng robot khi vận hành thực tế, tín hiệu đầu ra của bộ PID buộc phải đi qua các lớp bảo vệ thiết yếu trước khi được phát lệnh tới cơ cấu chấp hành:
\begin{itemize}
    \item \textbf{Khâu triệt vùng chết (Deadband):} Chủ động bỏ qua các vi sai quá nhỏ dao động quanh vị trí cân bằng ($0$ rad). Kỹ thuật này triệt tiêu hiện tượng chattering (động cơ rung lắc liên tục), qua đó giảm hao mòn cơ khí và tỏa nhiệt không cần thiết.
    \item \textbf{Bão hòa tín hiệu (Saturation) và Lọc thông thấp (LPF):} Lệnh điều khiển $u_{PID}$ được giới hạn biên độ trong khoảng an toàn $[-u_{max}, u_{max}]$ để ngăn chặn các xung bù vượt quá khả năng chịu tải của cơ hệ. Sau đó, tín hiệu được làm mượt qua bộ lọc thông thấp (Low-Pass Filter) để các khớp chuyển động êm ái, hạn chế tối đa các lực giật (jerk) cục bộ.
    \item \textbf{Chống tích lũy (Anti-windup):} Khi cơ cấu chấp hành hoặc tín hiệu đầu ra chạm ngưỡng bão hòa vật lý, cơ chế anti-windup sẽ tự động tạm dừng hoặc suy giảm thành phần tích phân $I$. Điều này chặn đứng sự tích lũy sai số ảo dư thừa, giúp hệ thống phục hồi nhanh chóng và không bị vọt lố (overshoot) khi các nhiễu loạn ngoại lực kết thúc.
\end{itemize}
